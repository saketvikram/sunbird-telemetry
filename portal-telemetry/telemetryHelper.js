...
telemtryEventConfig["pdata"]["ver"] = "1.0.0";
...
module.exports = {
  ...
  logSessionStart: function (req, callback) {
    var channel =
      req.session.rootOrghashTagId || _.get(req, "headers.X-Channel-Id");
  ...
  getParamsData: function (options, statusCode, resp, uri) {
    ...
    if (resp) {
      params.push({ rid: resp.id }, { size: resp.toString().length });
    }
    return params;
  },
  ...
  logAPICallEvent: function (req) {
    // function call is commented in all files
    ...
    if (req.id && req.type) {
      object = telemetry.getObjectData({
        id: req.id,
        type: req.type,
        ver: req.version,
        rollup: req.rollup,
    ...
    var channel =
      req.reqObj.session.rootOrghashTagId || _.get(req, "headers.X-Channel-Id");
    if (channel) {
  ...
  logGrantLogEvent: function (req) {
    const message = req.success
      ? "Verified keyclock grant"
      : "Keyclock grant failed";
  ...
  logAPIAccessEvent: function (req) {
    ...
    var channel =
      (req.reqObj &&
        req.reqObj.session &&
        req.reqObj.session.rootOrghashTagId) ||
      req.channel ||
      _.get(req, "headers.X-Channel-Id");
    if (channel) {
      var dims = _.clone(req.reqObj.session.orgs || []);
      dims = dims ? _.concat(dims, channel) : channel;
      const context = telemetry.getContextData({
  ...
  logAPIErrorEvent: function (req) {
    ...
    let object = {};
    const err =
      (req.resp && req.resp.params && req.resp.params.err) || "API_CALL_ERROR";
  ...
  logApiErrorEventV2: function (req, options) {
    ...
    let channel =
      req.session.rootOrghashTagId ||
      req.get("x-channel-id") ||
      envHelper.defaultChannelId;
  ...
  logSessionEvents: function (req, res) {
    if (req.body && req.body.event) {
      ...
      if (
        req.session["sessionEvents"].length >= parseInt(telemetryPacketSize, 10)
      ) {
    ...
    request(options, function (error, response, body) {
      if (_.isFunction(callback)) {
        ...
        if (error) {
          ...
          /* istanbul ignore next  */
          console.log(
            "telemetry sync error while syncing  portal",
            body.params.err
          );
          /* istanbul ignore next  */
          callback(body, false);
  ...
  getTelemetryActorData: function (req) {
    ...
    if (req.session && req.session.userId) {
      ...
      actor.id =
        req.headers["x-consumer-id"] || telemtryEventConfig.default_userid;
      actor.type =
        req.headers["x-consumer-username"] ||
        telemtryEventConfig.default_username;
    }
    return actor;
  },
  ...
  generateTelemetryForProxy: function (req, res, next) {
    ...
    var channel =
      (req.session && req.session.rootOrghashTagId) || req.get("x-channel-id");
    if (channel) {
